...
def show_item_details_modal():
    """Простое модальное окно с подробностями товара и изменением адреса"""
    ...
    if st.session_state.viewing_item is not None:
        ...
        with st.container(border=True):
            ...
            # Детальная информация в две колонки
...
- **Кол-во:** {item.get('Количество', 0)} шт
            ...
            with col_location:
                ...
                from database import WAREHOUSE_MAP, supabase
                import numpy as np
                ...
                # Выбор ячейки (по умолчанию текущий адрес если он в этом складе)
                default_idx = 0
            ...
            # Кнопка сохранения - сохраняет выбранный адрес
            if st.button("💾 СОХРАНИТЬ АДРЕС", use_container_width=True, type="primary"):
                try:
                    # Обновляем в таблице inventory
                    inv_payload = {
                        "doc_id": doc_id,
                        "item_name": item_name,
                    ...
                    st.session_state.viewing_item['Адрес'] = selected_cell
...
def show_inventory_database():
    """Вкладка База Данных с просмотром подробностей товара"""
    ...
    if inventory_df is None or (isinstance(inventory_df, pd.DataFrame) and inventory_df.empty):
        ...
        # При выборе строки показываем детали
        ...
        # Показываем модаль с подробностями
        st.divider()
...
TABLES_TO_LOAD = {
    "orders": "orders",
    "main": "main_registry", # Твой SQL VIEW
    "drivers": "drivers",
    "vehicles": "vehicles",
    "arrivals": "arrivals",
    "defects": "defects",
