...
# --- РАЗДЕЛ ВОДИТЕЛИ ---     
    ...
    # 4. Расход топлива (Инженерная модель: База + Коэффициент нагрузки)
        ...
        # --- 4. ПРОФЕССИОНАЛЬНАЯ ЛОГИКА РАСХОДА (ИНЖЕНЕРНЫЙ МЕТОД) ---
        base_rate = 9.0  # Базовая норма л/100км (установлено пользователем)
        
        # А. Коэффициент скорости (Аэродинамическое сопротивление)
        # На скоростях выше 90 км/ч расход растет не линейно, а квадратично.
        high_speed_df = df[df['speed_kmh'] > 90]
        if not high_speed_df.empty:
            # Считаем среднее превышение над лимитом
            avg_over_speed = high_speed_df['speed_kmh'].mean() - 90
            # Каждые 10 км/ч выше 90 добавляют ~15% к расходу
            speed_factor = 1 + (avg_over_speed / 10) * 0.15
        else:
            speed_factor = 1.0

        # Б. Коэффициент динамики (Разгоны/Торможения)
        # Вместо количества, считаем среднюю интенсивность ускорения
        df['accel_ms2'] = df['speed_kmh'].diff().fillna(0) / 3.6
        positive_accel = df[df['accel_ms2'] > 0.5]['accel_ms2'] # Только реальные разгоны
        
        if not positive_accel.empty:
            avg_accel = positive_accel.mean()
            # Нормальный разгон ~0.8-1.2 м/с2. Все что выше — агрессия.
            accel_factor = 1 + (max(0, avg_accel - 0.8) * 0.2)
        else:
            accel_factor = 1.0

        # В. Итоговый коэффициент нагрузки (Load Factor)
        # Ограничиваем сверху 40% (1.4), что реально для перегруженного буса или агрессивной езды
        load_factor = min(1.4, speed_factor * accel_factor)
        
        # Расчет итогов
        fuel_total = (actual_period_km / 100) * base_rate * load_factor
        cost_mdl = fuel_total * 21.0  # Цена 21 MDL за литр
