...
def create_arrival_modal(table_key="arrivals", *args, **kwargs):
    ...
    if uploaded_file:
        try:
            ...
            if name_col:
                ...
                if qty_col:
                    # СИНХРОНИЗАЦИЯ: Редактор ждет 'Кол-во'
                    df = df.rename(columns={qty_col: 'Кол-во'})
                
                # Колонки для полной совместимости с edit_arrival_modal
                if 'Объем (м3)' not in df.columns: df['Объем (м3)'] = 0.0
                if 'Адрес' not in df.columns: df['Адрес'] = "НЕ УКАЗАНО"
                
                parsed_items_df = df[['Название товара', 'Кол-во', 'Объем (м3)', 'Адрес']]
                st.success(f"✅ Товаров в файле: {len(df)}")
        except Exception as e:
            st.error(f"❌ Ошибка парсинга: {e}")

    # --- 2. ФОРМА ---
    with st.form("arrival_form_sync_final", clear_on_submit=False):
        col1, col2, col3 = st.columns([2, 1, 1])
        # Ключи client_name, doc_number — как в базе и редакторе
        client_name = col1.text_input("🏢 Поставщик*", placeholder="Название компании")
        ...
        # load_address — это "Склад приемки" в редакторе
        load_addr_val = r2_c3.text_input("🚪 Склад приемки", value="Склад №1")

        st.divider()
        st.markdown("### 📸 Фотофиксация")
        arrival_photo = st.file_uploader("Фото накладной или груза*", type=['png', 'jpg', 'jpeg'])
        
        comments_val = st.text_area("📝 Заметки")
    ...
    if submitted:
        ...
        # --- ИСПОЛЬЗУЕМ ТВОЙ UPLOADER.PY ---
        photo_url = None
        with st.spinner("☁️ Загрузка в Cloudinary через uploader.py..."):
            photo_url = upload_to_cloudinary(arrival_photo, folder_name="arrivals")
        
        if not photo_url:
            st.error("🚨 Не удалось загрузить фото. Проверьте st.secrets!")
            return

        # Подготовка данных (ПОЛНАЯ СИНХРОНИЗАЦИЯ С РЕДАКТОРОМ)
        items_list = parsed_items_df.replace({np.nan: None}).to_dict(orient='records')
        try:
            total_vol = round(float(parsed_items_df['Объем (м3)'].sum()), 3) if not parsed_items_df.empty else 0.0
        except:
            total_vol = 0.0
